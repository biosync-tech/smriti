...
pub fn handle_init(db_path: &str) -> AppResult<()> {
    ...
    // Initialize the database (`--db` is the global flag; CI uses it).
    let _db = Database::new(db_path)?;

    if let Some(parent) = abs_path.parent() {
        let cfg_path = parent.join("config.toml");
        if !cfg_path.exists() {
            let db_escaped = abs_path
                .display()
                .to_string()
                .replace('\\', "\\\\")
                .replace('"', "\\\"");
            let cfg = format!(
                "# Generated by `smriti init`. Safe to edit.\n\
                 db_path = \"{db_escaped}\"\n\
                 \n\
                 [consolidation]\n\
                 # conservative = flag schema proposals for human review (healthcare default)\n\
                 # standard/aggressive = retrieve-context proxy may auto-commit.\n\
                 # The proxy is not WikiSkill task-accuracy gating (arXiv:2608.27454).\n\
                 policy = \"conservative\"\n\
                 \n\
                 [inference]\n\
                 # Optional local LLM for schema abstraction. Uncomment to enable.\n\
                 # backend = \"ollama\"\n\
    ...
    // Any MCP client (Claude Desktop, Cursor, Codex, custom). Same file as CLI.
    println!("Add this to your MCP client config:\n");
    println!("{{");
    println!("  \"mcpServers\": {{");
    println!("    \"smriti\": {{");
    println!("      \"command\": \"smriti\",");
    let default = crate::cli::paths::default_db_path();
    if abs_path == default {
        println!("      \"args\": [\"mcp\"]");
    } else {
        println!(
    ...
    println!("Agents: call smriti_status, then notes_create / notes_search / retrieve_context.");
    println!("CLI (same database):");
...
pub async fn handle_consolidate(
    ...
    if let Some(note_id) = explain {
        ...
        if node_type == "schema" {
            ...
            if !json {
                if let Ok(lineage) = db.execute(|conn| {
                    crate::features::schema_formation::explain_schema(conn, &note_id)
                }) {
                    if !lineage.events.is_empty() {
                        println!("\nconsolidation_events:");
                        for e in &lineage.events {
                            println!("  [{}] {}", e.event_type, e.reason);
                        }
                    }
                }
            }
    ...
    let backend = if llm {
        match crate::inference::create_backend(&crate::inference::InferenceConfig::default()).await
        {
            Ok(b) => Some(b),
            Err(e) => {
                eprintln!(
                    "LLM backend unavailable ({e}). Clusters will be flagged, not silently promoted."
                );
                None
            }
        }
    ...
    if json {
        ...
        if dry_run {
            println!("\nNo changes persisted (dry run). Use --apply to persist.");
        } else if let Ok(pending) =
            db.execute(crate::features::schema_formation::list_pending_proposals)
        {
            if !pending.is_empty() {
                println!("\nPending schema proposals (human review):");
                for p in pending {
                    println!("  {}  {}  {}", p.id, p.title, p.rationale);
                }
                println!("  smriti approve <id>   /   smriti reject <id> -r \"...\"");
            }
        }
    }
...
pub fn handle_proposals(db: &Database, json: bool) -> AppResult<()> {
    use crate::features::schema_formation::list_pending_proposals;

    let proposals = db.execute(list_pending_proposals)?;
    ...
    for p in &proposals {
        ...
        println!("    episodes: {}", p.source_ids.join(", "));
    }
    println!("Accept:  smriti approve <proposal_id|episode_id>");
    println!("Reject:  smriti reject <proposal_id|episode_id> -r \"...\"");
    Ok(())
}

/// Approve a flagged schema proposal. Accepts a proposal id or a source episode id.
pub fn handle_approve_proposal(db: &Database, cluster_id: &str) -> AppResult<()> {
    use crate::features::schema_formation::{
        commit_proposal, resolve_pending_proposal, GatingSignal,
    };

    let formed = db.execute(|conn| {
        let proposal = resolve_pending_proposal(conn, cluster_id)?;
        commit_proposal(
            conn,
            &proposal,
            &GatingSignal::HumanApproved { by: "human".into() },
        )
    })?;

    println!("✓ Approved proposal → schema {}", formed.schema_id);
    println!("  Title:   {}", formed.title);
    println!("  Sources: {}", formed.source_ids.len());
    Ok(())
}

/// Reject a flagged schema proposal. Does not touch committed schema notes.
pub fn handle_reject_proposal(db: &Database, cluster_id: &str, reason: &str) -> AppResult<()> {
    use crate::features::schema_formation::{reject_proposal, resolve_pending_proposal};

    db.execute(|conn| {
        let proposal = resolve_pending_proposal(conn, cluster_id)?;
        reject_proposal(conn, &proposal, "human", reason)
    })?;

    println!("✓ Rejected proposal {cluster_id}");
